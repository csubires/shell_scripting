...
# --- Configuration ---
readonly SOURCE_CODE=("*.py")
readonly FLAKE8_IGNORE="W191,E501,F405,E203,W503"
readonly EXCLUDE_PATTERNS="*__pycache__* *test* *migrations* *venv* *.env*"

help_panel() {
    clear
    lg_prt "wtw" "\n" "PYTHON3 FORMAT" "\n"
    lg_prt "wobv" " Usage:" "./python_format.sh" "<Option>" "<Path>"
    ...
    lg_prt "c" "\t$0 -c main.py                Analyze main.py with cppcheck"
...
run_flake8() {
    ...
    while IFS= read -r file; do
        [[ ! -f "$file" ]] && continue
        local errors
        flake8 "$file" --ignore="$FLAKE8_IGNORE" --count
    done < <(find_files "$path" "${SOURCE_CODE[@]}")
    exit 0
}
...
autopep8_format() {
    ...
    if [[ -d "$target" ]]; then
        while IFS= read -r file; do
            [[ ! -f "$file" ]] && continue
            lg_prt "w" "Processing: $file"
            autopep8 --in-place --aggressive --ignore="$FLAKE8_IGNORE" "$file"
        done < <(find_files "$target" "${SOURCE_CODE[@]}")
    else
...
extract_includes() {
    ...
    local includes=$(grep -h 'import' "${files[@]}" 2>/dev/null | sort | uniq)
...
find_todos() {
    local directory="$1"
    [[ ! -d "$directory" ]] && { lg_prt "r" "Error: $directory is not a valid directory"; exit 1; }
    local todos=$(find "$directory" -type f \( -name "*.py" \) -exec \
                 egrep -n -i "TODO|TO-DO|TO DO|TO - DO" {} + 2>/dev/null || true)
    [[ -z "$todos" ]] && lg_prt "g" "No TODOs found!" || echo "$todos"
}

# Process arguments
[[ $# -eq 0 ]] && { help_panel; exit 1; }

# Parse options
case $1 in
    -f|--format)    [[ $# -eq 2 ]] && autopep8_format "$2" || lg_prt "r" "Error: Option -f requires an argument";;
    -c|--check)     [[ $# -eq 2 ]] && run_flake8 "$2" || lg_prt "r" "Error: Option -c requires an argument";;
